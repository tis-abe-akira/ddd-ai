...
class PartyControllerTest {
    ...
    void 存在しない企業を取得すると404が返る() throws Exception {
        setUp();
        mockMvc.perform(get("/api/v1/parties/companies/non-existent-id"))
    ...
    void 存在しない借り手を取得すると404が返る() throws Exception {
        setUp();
        mockMvc.perform(get("/api/v1/parties/borrowers/non-existent-id"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.status").value(404));
    }
    ...
    void 存在しない投資家を取得すると404が返る() throws Exception {
        setUp();
        mockMvc.perform(get("/api/v1/parties/investors/non-existent-id"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.status").value(404));
    }
}