...
interface FeePaymentTableProps {
  ...
  onDelete?: (feePayment: FeePayment) => void;
  isLoading?: boolean;
}
...
const FeePaymentTable: React.FC<FeePaymentTableProps> = ({
  ...
  const handleDeleteClick = (feePayment: FeePayment) => {
    if (window.confirm(
      `Are you sure you want to delete fee payment #${feePayment.id}?\n\n` +
                ...
                <tr className="hover:bg-secondary-600/50 transition-colors">
                  ...
                  <td className="px-4 py-4 text-center">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      {onDelete && isDeletable(feePayment) && (
                        <button
                          onClick={() => handleDeleteClick(feePayment)}
                          className="bg-error hover:bg-red-600 text-white font-medium py-1 px-2 rounded-lg transition-colors duration-200 text-sm"
