...
const BorrowerTable: React.FC<BorrowerTableProps> = ({ onRefresh, refreshTrigger, onEdit, onDelete }) => {
            ...
            <tbody className="bg-primary-900 divide-y divide-secondary-500">
              {filteredBorrowers.length === 0 ? (
                ...
                filteredBorrowers.map((borrower) => (
                  <tr key={borrower.id} className="hover:bg-secondary-600/30 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-white">{borrower.name}</div>
                      <div className="text-sm text-accent-400">ID: {borrower.id}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-secondary-600 text-accent-400">
                        Borrower
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`text-sm font-medium ${getCreditRatingColor(borrower.creditRating)}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        {onEdit && (
                          <button
                            onClick={() => {
                              console.log('Edit borrower clicked!', borrower);
                              onEdit(borrower);
                            }}
                        ...
                        <button
                          ...
                          className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium ${
                            pageNumber === pagination.page
                              ? 'z-10 bg-accent-500 border-accent-500 text-primary-900'
